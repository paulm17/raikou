...
function unocss(options = {}) {
  const { cwd = process__default.cwd(), configOrPath } = options;
    ...
    plugins: [
      async function(root, result) {
        ...
        try {
          ...
          if (!uno) {
            ...
            if (config_mtime > last_config_mtime) {
              uno = core.createGenerator(
                (await config.loadConfig(cwd, configOrPath)).config
              );
        ...
        if (options.purgeStyles) {
          const purge_css = postcss__default.parse(
          ...
          pagesContent.clear();
          const importedModules = await fsPromises__default.readFile(
