...
describe('@raikou/dropzone/Dropzone', () => {
  tests.itSupportsSystemProps<DropzoneProps, DropzoneStylesNames>({
    ...
    displayName: '@raikou/dropzone/Dropzone',
  ...
  it('displays LoadingOverlay based on loading prop', () => {
    ...
    expect(container.querySelectorAll('.raikou-LoadingOverlay-root')).toHaveLength(0);

    rerender(<Dropzone {...defaultProps} loading />);
    expect(container.querySelectorAll('.raikou-LoadingOverlay-root')).toHaveLength(1);
  });
