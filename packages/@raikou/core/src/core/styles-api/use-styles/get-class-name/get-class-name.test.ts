...
const THEME_WITH_CLASSNAMES: RaikouTheme = {
  ...DEFAULT_THEME,
  components: {
    TestComponentObject: {
      classNames: {
...
describe('@raikou/core/get-class-name', () => {
  ...
  it('resolves static selectors correctly', () => {
    ...
    expect(output.includes('raikou-Test-root')).toBe(true);
    expect(output.includes('raikou-AnotherTest-root')).toBe(true);
  });
  ...
  it('does not add static classes if withStaticClasses is false', () => {
    ...
    expect(output.includes('raikou-Test-root')).toBe(false);
    expect(output.includes('raikou-AnotherTest-root')).toBe(false);
