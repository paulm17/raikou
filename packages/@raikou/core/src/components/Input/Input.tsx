...
const varsResolver = createVarsResolver<InputFactory>((_, props, ctx) => ({
  wrapper: {
    '--input-margin-top': ctx.offsetTop ? 'calc(var(--raikou-spacing-xs) / 2)' : undefined,
    '--input-margin-bottom': ctx.offsetBottom ? 'calc(var(--raikou-spacing-xs) / 2)' : undefined,
...
export const Input = polymorphicFactory<InputFactory>((_props, ref) => {
  ...
  const getStyles = useStyles<InputFactory>({
    ...
    classes: {
      ...
      section: InputSectionStyle,
      label: InputLabelStyle,
      required: InputRequiredStyle,
      description: InputDescriptionStyle,
