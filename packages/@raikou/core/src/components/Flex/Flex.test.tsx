...
describe('@raikou/core/Flex', () => {
  tests.itSupportsSystemProps<FlexProps, FlexStylesNames>({
    ...
    displayName: '@raikou/core/Flex',
    stylesApiSelectors: ['root'],
  });

  it('supports gap style prop', () => {
    const { container } = render(<Flex gap="2em" />);
    expect(container.querySelector('.raikou-Flex-root')).toHaveStyle({ gap: '2em' });
  });

  it('supports rowGap style prop', () => {
    const { container } = render(<Flex rowGap="2em" />);
    expect(container.querySelector('.raikou-Flex-root')).toHaveStyle({ rowGap: '2em' });
  });

  it('supports columnGap style prop', () => {
    const { container } = render(<Flex columnGap="2em" />);
    expect(container.querySelector('.raikou-Flex-root')).toHaveStyle({ columnGap: '2em' });
  });

  it('supports align style prop', () => {
    const { container } = render(<Flex align="center" />);
    expect(container.querySelector('.raikou-Flex-root')).toHaveStyle({ alignItems: 'center' });
  });

  it('supports justify style prop', () => {
    const { container } = render(<Flex justify="center" />);
    expect(container.querySelector('.raikou-Flex-root')).toHaveStyle({
  ...
  it('supports wrap style prop', () => {
    ...
    expect(container.querySelector('.raikou-Flex-root')).toHaveStyle({ flexWrap: 'wrap' });
  });
