...
export const CompositeChart = factory<CompositeChartFactory>((_props, ref) => {
  ...
  const getStyles = useStyles<CompositeChartFactory>({
    ...
    classes: {
      ...
      axis: GridChartAxisStyle,
      grid: GridChartGridStyle,
      axisLabel: GridChartAxisLabelStyle,
      tooltip: GridChartTooltipStyle,
    },
    props,
    className,
    style,
    ...
    if (item.type === 'line') {
      return (
        <Line
          ...
          activeDot={
            withDots
              ? { fill: 'var(--raikou-color-white)', stroke: color, ...activeDotProps }
              : false
          }
          fill={color}
          stroke={color}
...
CompositeChart.displayName = '@raikou/charts/CompositeChart';
