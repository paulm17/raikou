...
export interface SparklineTrendColors {
  positive: RaikouColor;
  negative: RaikouColor;
  neutral?: RaikouColor;
}
      ...
      <ResponsiveContainer>
        <ReChartsAreaChart data={mappedData}>
          <Area
            ...
            type={curveType}
            fill={`url(#${id})`}
            stroke="var(--chart-color, var(--raikou-color-blue-filled))"
          ...
          <defs>
            <AreaGradient
              id={id}
              color="var(--chart-color, var(--raikou-color-blue-filled))"
...
Sparkline.displayName = '@raikou/charts/Sparkline';
