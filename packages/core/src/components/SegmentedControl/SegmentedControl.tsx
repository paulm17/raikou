...
import { useId, useMergedRef, useMounted, useUncontrolled } from '@raikou/hooks';
import {
  Box,
  BoxProps,
  createVarsResolver,
...
export const SegmentedControl = factory<SegmentedControlFactory>((_props, ref) => {
  ...
  const getStyles = useStyles<SegmentedControlFactory>({
    ...
    props,
    classes: {
      root: SegmentedControlRootStyle,
      control: SegmentedControlControlStyle,
      input: SegmentedControlInputStyle,
      label: SegmentedControlLabelStyle,
      indicator: SegmentedControlIndicatorStyle,
      innerLabel: SegmentedControlInnerLabelStyle,
    },
    className,
    style,
    classNames,
  ...
  const mergedRef = useMergedRef(ref, (node: any) => setParent(node));
