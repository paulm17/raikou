...
describe("@raikou/hooks/use-queue", () => {
  ...
  it("correctly applies given update to state without queue", () => {
    const hook = renderHook(() =>
      useQueue({ initialValues: [1, 2], limit: 3 })
    );
    act(() =>
      hook.result.current.update((state) => state.filter((i) => i % 2))
    );
  ...
  it("correctly applies given update to state with queue", () => {
    const hook = renderHook(() =>
      useQueue({ initialValues: [1, 2, 3, 4, 5, 6, 7, 8], limit: 3 })
    );
    act(() =>
      hook.result.current.update((state) => state.filter((i) => i % 2))
    );
  ...
  it("puts extra items to the queue if state has extra items after update", () => {
    const hook = renderHook(() =>
      useQueue<number>({ initialValues: [], limit: 3 })
    );
  ...
  it("cleans queue with cleanQueue handlers", () => {
    const hook = renderHook(() =>
      useQueue({ initialValues: [1, 2, 3, 4], limit: 2 })
    );
