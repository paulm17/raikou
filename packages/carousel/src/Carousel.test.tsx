...
describe('@raikou/carousel/Carousel', () => {
  tests.itSupportsSystemProps<CarouselProps, CarouselStylesNames>({
    ...
    displayName: '@raikou/carousel/Carousel',
    stylesApiSelectors: [
      'root',
      'container',
      'control',
  ...
  it('renders controls if withControls is true', () => {
    ...
    expect(container.querySelector('.raikou-Carousel-controls')).toBeInTheDocument();
  });

  it('does not render controls if withControls is false', () => {
    const { container } = render(<Carousel withControls={false} />);
    expect(container.querySelector('.raikou-Carousel-controls')).not.toBeInTheDocument();
  });

  it('renders indicators if withIndicators is true', () => {
    const { container } = render(<Carousel withIndicators />);
    expect(container.querySelector('.raikou-Carousel-indicators')).toBeInTheDocument();
  });

  it('does not render indicators if withIndicators is false', () => {
    const { container } = render(<Carousel withIndicators={false} />);
    expect(container.querySelector('.raikou-Carousel-indicators')).not.toBeInTheDocument();
  });
