...
export const SpotlightActionsListStyle = css(({ theme }: any) => ({
  ...
  ...theme.applyStyles('light', {
    borderColor: theme.vars.colors.gray[2],
  }),
  ...theme.applyStyles('dark', {
    borderColor: theme.vars.colors.dark[4],
  }),
}));

export const SpotlightActionStyle = css(({ theme }: any) => ({
  display: 'flex',
  alignItems: 'center',
  width: '100%',
  ...
  '&:_where(:not([data-selected]))': {
    ...theme.applyMixin('hover', {
      "&:hover": {
        ...
        ...theme.applyStyles('dark', {
          backgroundColor: theme.vars.colors.dark[6],
        }),
      }
    }),
  },
...
export const SpotlightActionDescriptionStyle = css(({ theme }: any) => ({
  display: 'block',
...
export const SpotlightEmptyStyle = css(({ theme }: any) => ({
  color: 'var(--raikou-color-dimmed)',
  padding: 'var(--raikou-spacing-md)',
  textAlign: 'center',
}));

export const SpotlightFooterStyle = css(({ theme }: any) => ({
