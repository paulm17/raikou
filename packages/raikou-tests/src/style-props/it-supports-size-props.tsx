...
export function itSupportsSizeProps<Props>(
  ...
  it(name, () => {
    const { container: theme } = render(
      <options.component {...options.props} w="xl" />
    );
    const { container: w } = render(
      <options.component {...options.props} w="10%" />
    );
    const { container: miw } = render(
      <options.component {...options.props} miw="10vh" />
    );
    const { container: maw } = render(
      <options.component {...options.props} maw="20%" />
    );
    const { container: h } = render(
      <options.component {...options.props} h="10%" />
    );
    const { container: mih } = render(
      <options.component {...options.props} mih="10vh" />
    );
    const { container: mah } = render(
      <options.component {...options.props} mah="20%" />
    );

    expect(theme.querySelector(selector)).toHaveStyle({
      width: "var(--raikou-spacing-xl)",
    });
