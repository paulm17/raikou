...
return false;}
import React from"react";function isElement(value){if(Array.isArray(value)||value===null){return false;}
if(typeof value==="object"){if(value.type===React.Fragment){return false;}
return true;}
return false;}
...
import cx from"clsx";var EMPTY_CLASS_NAMES={};function mergeClassNames(objects){const merged={};objects.forEach((obj)=>{Object.entries(obj).forEach(([key,value])=>{if(typeof value==="object"){let newValue="";Object.entries(value).forEach(([subKey,subValue])=>{newValue+=`${subKey}-${subValue}`;});value=newValue.trim();}
if(merged[key]){merged[key]=cx(merged[key],value);}else{merged[key]=value;}});});return merged;}
...
import cx3 from"clsx";function getStaticClassNames({themeName,classNamesPrefix,selector}){return themeName.map((n)=>`${n.toLowerCase()}-${selector}`);}
...
import React2 from"react";function cssObjectToString(css){return keys(css).reduce((acc,rule)=>css[rule]!==void 0?`${acc}
${camelToKebabCase(rule)}:${css[rule]};`:acc,"").trim();}
function padLines(string,count){const whitespace=" ".repeat(count);return`${whitespace}${string.replace(/(?:\r\n|\r|\n)/g,`${whitespace}`)}`;}
function stylesToString({selector,styles,media}){const baseStyles=styles?padLines(cssObjectToString(styles),2):"";const mediaQueryStyles=!Array.isArray(media)?[]:media.map((item)=>`@media ${item.query}{${selector}{${padLines(cssObjectToString(item.styles),4)}}}`);return`${baseStyles?`${selector}{${baseStyles}}`:""}${mediaQueryStyles.join("\n\n")}`.trim();}
function InlineStyles({selector,styles,media}){return React2.createElement("style",{"data-raikou-styles":"inline",dangerouslySetInnerHTML:{__html:stylesToString({selector,styles,media})}});}
...
import{useId}from"react";function useRandomClassName(){const id=useId().replace(/:/g,"");return`__m__-${id}`;}
function getStyleObject(style,theme){if(Array.isArray(style)){return[...style].reduce((acc,item)=>({...acc,...getStyleObject(item,theme)}),{});}
if(typeof style==="function"){return style(theme);}
if(style==null){return{};}
return style;}
import React3,{forwardRef as forwardRef3}from"react";import cx4 from"clsx";import{forwardRef}from"react";function identity(value){return value;}
function factory(ui){const Component=forwardRef(ui);Component.extend=identity;return Component;}
import{forwardRef as forwardRef2}from"react";function polymorphicFactory(ui){const Component=forwardRef2(ui);Component.extend=identity;return Component;}
