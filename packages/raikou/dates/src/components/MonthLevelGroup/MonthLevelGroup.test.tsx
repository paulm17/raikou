...
describe("@raikou/dates/MonthLevelGroup", () => {
  tests.itSupportsSystemProps<MonthLevelGroupProps, MonthLevelGroupStylesNames>(
    {
      ...
      displayName: "@raikou/dates/MonthLevelGroup",
      stylesApiSelectors: [
        "calendarHeader",
  ...
  it("renders correct number of months based on numberOfColumns prop", () => {
    const { rerender } = render(
      <MonthLevelGroup {...defaultProps} numberOfColumns={1} />,
    );
  ...
  it("renders correct months group based on month prop", () => {
    ...
    ).toStrictEqual(["April 2022", "May 2022", "June 2022"]);
  });

  it("supports levelControlAriaLabel as string", () => {
    render(
      <MonthLevelGroup {...defaultProps} levelControlAriaLabel="test-label" />,
    );
    expect(screen.getByText("April 2022")).toHaveAttribute(
      "aria-label",
      "test-label",
    );
  });
