...
describe("@raikou/dates/Day", () => {
  ...
  it("renders given date value", () => {
    ...
    expect(screen.getByRole("button")).toHaveTextContent(
      defaultProps.date.getDate().toString(),
    );
  });
  ...
  it("has correct default __staticSelector", () => {
    render(<Day {...defaultProps} />);
    expect(screen.getByRole("button")).toHaveClass("raikou-Day-day");
  });

  it("supports __staticSelector", () => {
    render(<Day {...defaultProps} __staticSelector="Month" />);
    expect(screen.getByRole("button")).toHaveClass("raikou-Month-day");
  });
  ...
  it("supports static prop", () => {
    ...
    expect(
      (container.querySelector("[data-weekend]") as HTMLElement).tagName,
    ).toBe("BUTTON");
  ...
  it("adds data-today attribute if date is the same as today", () => {
    const { rerender } = render(
      <Day {...defaultProps} date={new Date(2021, 11, 1)} />,
    );
