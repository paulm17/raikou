...
import { userEvent, render, screen } from "@raikou/tests";
...
describe("@raikou/dates/PickerInputBase", () => {
  ...
  it("supports __staticSelector", () => {
    const { container } = render(
      <PickerInputBase
        {...defaultProps}
        __staticSelector="TestStaticSelector"
      />,
    );
    expect(container.querySelector("*:not(style)")).toHaveClass(
      "raikou-TestStaticSelector-root",
    );
    expect(screen.getByText("test-value")).toHaveClass(
      "raikou-TestStaticSelector-input",
    );
  });
  ...
  it("changes between Popover and Modal based on dropdownType prop", () => {
    ...
    expect(
      container.querySelector(".raikou-Popover-dropdown"),
    ).toBeInTheDocument();
    expect(
      container.querySelector(".raikou-Modal-content"),
    ).not.toBeInTheDocument();
    expect(screen.getByText("test-children")).toBeInTheDocument();

    rerender(
      <PickerInputBase {...defaultProps} dropdownOpened dropdownType="modal" />,
    );
    expect(
      container.querySelector(".raikou-Popover-dropdown"),
    ).not.toBeInTheDocument();
