...
describe("@raikou/dates/DecadeLevelGroup", () => {
  ...
  >({
    ...
    displayName: "@raikou/dates/DecadeLevelGroup",
    stylesApiSelectors: [
      "levelsGroup",
      "calendarHeader",
  ...
  datesTests.itHandlesControlsKeyboardEvents({
    component: DecadeLevelGroup,
    props: defaultProps,
    listSelector: ".raikou-DecadeLevelGroup-yearsList",
  });

  it("renders correct number of columns based on numberOfColumns prop", () => {
    const { rerender } = render(
      <DecadeLevelGroup {...defaultProps} numberOfColumns={1} />,
    );
  ...
  it("renders correct years group based on year prop", () => {
    ...
    expect(
      screen.getAllByLabelText("level-control").map((node) => node.textContent),
    ).toStrictEqual(["2020 – 2029", "2030 – 2039", "2040 – 2049"]);
  });

  it("supports levelControlAriaLabel as string", () => {
    render(
      <DecadeLevelGroup {...defaultProps} levelControlAriaLabel="test-label" />,
    );
    expect(screen.getByText("2020 – 2029")).toHaveAttribute(
      "aria-label",
      "test-label",
    );
  });
  ...
  it("has correct default __staticSelector", () => {
    ...
    expect(container.querySelector("table button")).toHaveClass(
      "raikou-DecadeLevelGroup-yearsListControl",
    );
  });
