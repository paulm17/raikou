...
describe("@raikou/dates/YearLevelGroup", () => {
  tests.itSupportsSystemProps<YearLevelGroupProps, YearLevelGroupStylesNames>({
    ...
    displayName: "@raikou/dates/YearLevelGroup",
    stylesApiSelectors: [
      "levelsGroup",
      "calendarHeader",
  ...
  datesTests.itHandlesControlsKeyboardEvents({
    component: YearLevelGroup,
    props: defaultProps,
    listSelector: ".raikou-YearLevelGroup-monthsList",
  });

  it("renders correct number of columns based on numberOfColumns prop", () => {
    const { rerender } = render(
      <YearLevelGroup {...defaultProps} numberOfColumns={1} />,
    );
  ...
  it("renders correct years group based on year prop", () => {
    ...
    ).toStrictEqual(["2022", "2023", "2024"]);
  });

  it("supports levelControlAriaLabel as string", () => {
    render(
      <YearLevelGroup {...defaultProps} levelControlAriaLabel="test-label" />,
    );
    expect(screen.getByText("2022")).toHaveAttribute(
      "aria-label",
      "test-label",
    );
  });
  ...
  it("has correct default __staticSelector", () => {
    ...
    expect(container.querySelector("table button")).toHaveClass(
      "raikou-YearLevelGroup-monthsListControl",
    );
  });
