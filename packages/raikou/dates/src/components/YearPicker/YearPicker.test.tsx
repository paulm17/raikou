...
describe("@mantine/dates/YearPicker", () => {
  ...
  it("supports onClick handler from getYearControlProps", async () => {
    ...
    const { container } = render(
      <YearPicker
        {...defaultProps}
        getYearControlProps={() => ({ onClick: spy })}
      />,
    );
    await userEvent.click(container.querySelector("table button")!);
  ...
  it("only adds selected year of decade to tab order", async () => {
    ...
    await userEvent.tab();
    expect(
      screen.getByRole("button", { name: ariaLabels.previousDecade }),
    ).toHaveFocus();

    await userEvent.tab();
    expect(
      screen.getByRole("button", { name: ariaLabels.nextDecade }),
    ).toHaveFocus();

    await userEvent.tab();
    expect(
      screen.getByRole("button", {
        name: new Date(2013, 0).getFullYear().toString(),
      }),
    ).toHaveFocus();
  ...
  it("only adds current year of decade to tab order", async () => {
    ...
    await userEvent.tab();
    expect(
      screen.getByRole("button", { name: new Date().getFullYear().toString() }),
    ).toHaveFocus();
  ...
  it("only adds first non-disabled year of decade to tab order", async () => {
    ...
    expect(
      screen.getByRole("button", {
        name: new Date(2015, 0).getFullYear().toString(),
      }),
    ).toHaveFocus();
