...
describe("@raikou/dates/YearLevel", () => {
  tests.itSupportsSystemProps<YearLevelProps, YearLevelStylesNames>({
    ...
    displayName: "@raikou/dates/YearLevel",
    stylesApiSelectors: [
      "calendarHeader",
  ...
  it("has correct default __staticSelector", () => {
    ...
    expect(container.querySelector("table td button")).toHaveClass(
      "raikou-YearLevel-monthsListControl",
    );
    expect(screen.getByLabelText("level-control")).toHaveClass(
      "raikou-YearLevel-calendarHeaderLevel",
    );
  });

  it("supports custom __staticSelector", () => {
    const { container } = render(
      <YearLevel {...defaultProps} __staticSelector="Calendar" />,
    );
    expect(container.querySelector("table td button")).toHaveClass(
