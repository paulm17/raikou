...
import { render, tests, screen, userEvent } from "@raikou/tests";
...
describe("@raikou/dates/Calendar", () => {
  tests.itSupportsSystemProps<CalendarProps, CalendarStylesNames>({
    ...
    displayName: "@raikou/dates/Calendar",
    stylesApiSelectors: [
      "levelsGroup",
      "calendarHeader",
  ...
  datesTests.itHandlesControlsKeyboardEvents(
    {
      ...
      listSelector: ".raikou-Calendar-monthsList",
    },
  ...
  datesTests.itHandlesControlsKeyboardEvents(
    {
      ...
      listSelector: ".raikou-Calendar-yearsList",
    },
  ...
  it("sets correct aria-labels based on ariaLabels prop", () => {
    ...
    rerender(
      <Calendar {...defaultProps} ariaLabels={testLabels} level="year" />,
    );
    ...
    rerender(
      <Calendar {...defaultProps} ariaLabels={testLabels} level="decade" />,
    );
  ...
  it("calls onLevelChange when level changes", async () => {
    ...
    const { container } = render(
      <Calendar {...defaultProps} onLevelChange={spy} />,
    );
  ...
  it("does not change date when next/previous controls are clicked with date prop (controlled)", async () => {
    ...
    rerender(
      <Calendar {...defaultProps} date={new Date(2022, 3, 11)} level="year" />,
    );
    expectHeaderLevel("year", "2022");
    await clickNext("year");
    expectHeaderLevel("year", "2022");
  ...
  it("supports changing decade label format", () => {
    render(
      <Calendar {...defaultProps} level="decade" decadeLabelFormat="MM/YY" />,
    );
  ...
  it("only adds selected date in month to tab order", async () => {
    ...
    expect(
      screen.getByRole("button", {
        name: defaultProps.ariaLabels!.previousMonth,
      }),
    ).toHaveFocus();
    ...
    expect(
      screen.getByRole("button", {
        name: defaultProps.ariaLabels!.monthLevelControl,
      }),
    ).toHaveFocus();

    await userEvent.tab();
    expect(
      screen.getByRole("button", { name: defaultProps.ariaLabels!.nextMonth }),
    ).toHaveFocus();

    await userEvent.tab();
    expect(
      screen.getByRole("button", {
        name: dayjs(defaultProps.defaultDate).format("D MMMM YYYY"),
      }),
    ).toHaveFocus();
  ...
  it("only adds first non-disabled date in month to tab order", async () => {
    render(
      <Calendar
        ...
        getDayProps={(date) => ({
          disabled: dayjs(new Date(2022, 3, 15)).isSame(date, "date"),
        })}
      />,
    );
    ...
    expect(
      screen.getByRole("button", {
        name: dayjs(new Date(2022, 3, 16)).format("D MMMM YYYY"),
      }),
    ).toHaveFocus();
