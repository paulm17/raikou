...
describe("@raikou/form/onSubmit", () => {
  it("calls handleSubmit with values and event when all values are valid", () => {
    const hook = renderHook(() =>
      useForm({
        initialValues: { banana: "test banana", apple: "test apple" },
      }),
    );
  ...
  it("calls handleValidationFailure when values are not valid", () => {
    const hook = renderHook(() =>
      useForm({
        ...
        validate: {
          banana: (value) =>
            value !== "test-banana" ? "invalid banana" : null,
          orange: (value) =>
            value !== "test-orange" ? "invalid orange" : null,
        },
      }),
    );
