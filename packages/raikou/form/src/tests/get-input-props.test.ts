...
describe("@raikou/form/get-input-props", () => {
  ...
  it("updates form value with returned onChange handler (root property)", () => {
    const hook = renderHook(() =>
      useForm({ initialValues: { fruit: true, vegetable: "potato" } }),
    );

    act(() =>
      hook.result.current
        .getInputProps("fruit", { type: "checkbox" })
        .onChange(false),
    );
    ...
    act(() =>
      hook.result.current.getInputProps("vegetable").onChange("carrot"),
    );
    expect(hook.result.current.values).toStrictEqual({
      fruit: false,
      vegetable: "carrot",
  ...
  it("updates form value with returned onChange handler (nested object)", () => {
    const hook = renderHook(() =>
      useForm({
        initialValues: { nested: { fruit: true, vegetable: "potato" } },
      }),
    );

    act(() =>
      hook.result.current
        .getInputProps("nested.fruit", { type: "checkbox" })
        .onChange(false),
    );
    expect(hook.result.current.values).toStrictEqual({
    ...
    act(() =>
      hook.result.current.getInputProps("nested.vegetable").onChange("carrot"),
    );
    expect(hook.result.current.values).toStrictEqual({
  ...
  it("updates form value with returned onChange handler (nested array)", () => {
    ...
    act(() =>
      hook.result.current
        .getInputProps("nested.1.fruit", { type: "checkbox" })
        .onChange(false),
    );
    expect(hook.result.current.values).toStrictEqual({
      nested: [
    ...
    act(() =>
      hook.result.current
        .getInputProps("nested.0.vegetable")
        .onChange("carrot"),
    );
    expect(hook.result.current.values).toStrictEqual({
      nested: [
  ...
  it("returns onFocus if withFocus is true", () => {
    ...
    expect(typeof hook.result.current.getInputProps("a").onFocus).toBe(
      "function",
    );
    expect(
      typeof hook.result.current.getInputProps("a", { withFocus: false })
        .onFocus,
    ).toBe("undefined");
  });
});
