...
function validateFormName(name: string) {
  if (!/^[0-9a-zA-Z-]+$/.test(name)) {
    throw new Error(
      `[@raikou/use-form] Form name "${name}" is invalid, it should contain only letters, numbers and dashes`,
    );
  }
}
...
>(name: string) {
  ...
  const setErrors: SetErrors = (errors) =>
    dispatchEvent(`mantine-form:${name}:set-errors`, errors);
  ...
  const clearErrors: ClearErrors = () =>
    dispatchEvent(`mantine-form:${name}:clear-errors`);

  const reset: Reset = () => dispatchEvent(`mantine-form:${name}:reset`);

  const validate: () => void = () =>
    dispatchEvent(`mantine-form:${name}:validate`);
  ...
  const insertListItem: InsertListItem<FormValues> = (path, item, index) =>
    dispatchEvent(`mantine-form:${name}:insert-list-item`, {
      path,
      index,
      item,
    });
  ...
  const setDirty: SetFormStatus = (value) =>
    dispatchEvent(`mantine-form:${name}:set-dirty`, value);
  ...
  const resetTouched: ResetStatus = () =>
    dispatchEvent(`mantine-form:${name}:reset-touched`);
...
>(name: string | undefined, form: UseFormReturnType<Values, TransformValues>) {
  ...
  useFormEvent(
    ...
    (event: CustomEvent) => form.setInitialValues(event.detail),
  );
