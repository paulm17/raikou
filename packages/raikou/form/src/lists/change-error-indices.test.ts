...
describe("@raikou/form/change-error-indices", () => {
  ...
  it("increments deeply nested errors", () => {
    expect(
      changeErrorIndices("nested.0.inner", 2, TEST_ERRORS, 1),
    ).toStrictEqual({
      name: "name-error",
      "fruits.0.name": "fruit-error-1",
      "fruits.0.available": "fruit-error-2",
  ...
  describe("returns unchanged object", () => {
    it("if index is undefined", () => {
      expect(
        changeErrorIndices("fruits", undefined, TEST_ERRORS, 1),
      ).toStrictEqual(TEST_ERRORS);
    });

    it("if path is not a string", () => {
      expect(changeErrorIndices(1, 1, TEST_ERRORS, 1)).toStrictEqual(
        TEST_ERRORS,
      );
    });
    ...
    it("if path does not exist", () => {
      expect(
        changeErrorIndices("does-not-exist", 1, TEST_ERRORS, 1),
      ).toStrictEqual(TEST_ERRORS);
    });

    it("if index is bigger than any error index", () => {
      expect(changeErrorIndices("fruits", 100, TEST_ERRORS, 1)).toStrictEqual(
        TEST_ERRORS,
      );
    });
  });
