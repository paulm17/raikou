...
function createTheme(config) {
  const raikouColors = Object.keys(colors).reduce((acc, key) => {
    ...
    if ([...defaultColors, ...unocssColors].includes(key)) {
      Object.keys(colors[key]).forEach((shade) => {
        const isIncluded = (!config.raikouTheme.includeColors || config.raikouTheme.includeColors.includes(key)) && (!config.raikouTheme.excludeColors || !config.raikouTheme.excludeColors.includes(key));
        if (isIncluded) {
          if (!acc[key]) {
            acc[key] = [];
          }
  ...
  const theme = __spreadValues(__spreadValues({}, raikouTheme), config.raikouTheme);
  return theme;
}
