...
describe("@raikou/core/Highlight/highlighter", () => {
  ...
  it("returns initial string if highlight is empty", () => {
    expect(highlighter(VALUE, "")).toStrictEqual([
  ...
  it("does not highlight if nothing found", () => {
    expect(highlighter(VALUE, "Hi, there")).toStrictEqual([
      { chunk: VALUE, highlighted: false },
    ]);
  });

  it("does not highlight if highlight is null or undefined", () => {
    expect(highlighter(VALUE, null!)).toStrictEqual([
      { chunk: VALUE, highlighted: false },
    ]);
    expect(highlighter(VALUE, undefined!)).toStrictEqual([
      { chunk: VALUE, highlighted: false },
    ]);
  });
