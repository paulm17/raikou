...
import { tests, render, userEvent, screen } from "@raikou/tests";
import { DirectionProvider } from "@raikou/core";
...
const expectActiveTab = (value: TabValue | null) => {
  ...
  hidden.forEach((panel) => {
    expect(screen.queryByText(`${panel} panel`)).toHaveStyle({
      display: "none",
    });
  });

  if (value) {
    expect(screen.getByText(`${value} panel`)).not.toHaveStyle({
      display: "none",
    });
  }
};
