...
describe("@raikou/core/Modal", () => {
  ...
  it("sets data-centered attribute when centered prop is passed", () => {
    ...
    expect(container.querySelector(".raikou-Modal-root")).toHaveAttribute(
      "data-centered"
    );

    rerender(<Modal {...defaultProps} centered={false} />);
    expect(container.querySelector(".raikou-Modal-root")).not.toHaveAttribute(
      "data-centered"
    );
  });
  ...
  it("does not render overlay when withOverlay is false", () => {
    const { container, rerender } = render(
      <Modal {...defaultProps} withOverlay={false} />
    );
    expect(
      container.querySelector(".raikou-Modal-overlay")
  ...
  it("does not render header if title and withCloseButton are not provided", () => {
    ...
    rerender(
      <Modal {...defaultProps} withCloseButton={false} title="test-title" />
    );
    expect(container.querySelector(".raikou-Modal-header")).toBeInTheDocument();

    rerender(<Modal {...defaultProps} withCloseButton={false} title={null} />);
    expect(
      container.querySelector(".raikou-Modal-header")
    ).not.toBeInTheDocument();
  });
  ...
  it("renders given title", () => {
    ...
    expect(container.querySelector(".raikou-Modal-title")).toHaveTextContent(
      "test-title"
    );
  });
