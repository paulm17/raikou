...
describe("@raikou/core/InputLabel", () => {
  tests.itSupportsSystemProps<InputLabelProps, InputLabelStylesNames>({
    ...
    displayName: "@raikou/core/InputLabel",
  ...
  it("renders required asterisk if required prop is set", () => {
    ...
    expect(container.querySelector(".raikou-InputWrapper-required")).toBeNull();
  });

  it("changes root element based on labelElement prop", () => {
    const { rerender } = render(
      <InputLabel labelElement="label">Label</InputLabel>
    );
  ...
  it("sets htmlFor attribute if labelElement is label", () => {
    const { container, rerender } = render(
      <InputLabel labelElement="label" htmlFor="test" />
    );
