...
const getInputWrapper = (element: HTMLElement) =>
  element.querySelector(".raikou-Input-wrapper")!;

describe("@raikou/core/Input", () => {
  tests.axe([
  ...
  tests.itSupportsSystemProps<InputProps, InputStylesNames>({
    ...
    polymorphicSelector: ".raikou-Input-input",
    displayName: "@raikou/core/Input",
  ...
  it("sets data-with-left-section attribute based on leftSection prop", () => {
    ...
    expect(getInputWrapper(container)).toHaveAttribute(
      "data-with-left-section"
    );

    rerender(<Input leftSection={null} />);
    expect(getInputWrapper(container)).not.toHaveAttribute(
      "data-with-left-section"
    );
  });
