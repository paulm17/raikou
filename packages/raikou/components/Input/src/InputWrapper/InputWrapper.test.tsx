...
import { render, tests, inputWrapperQueries } from "@raikou/tests";
...
describe("@raikou/core/InputWrapper", () => {
  tests.itSupportsSystemProps<InputWrapperProps, InputWrapperStylesNames>({
    ...
    displayName: "@raikou/core/InputWrapper",
  ...
  it("generates correct ids for description and error", () => {
    const { container } = render(
      <InputWrapper {...defaultProps} id="test45" />
    );
    expect(inputWrapperQueries.getDescription(container)).toHaveAttribute(
      "id",
      "test45-description"
    );
    expect(inputWrapperQueries.getError(container)).toHaveAttribute(
      "id",
      "test45-error"
    );
  });
  ...
  it("sets data-variant attribute based on variant prop", () => {
    const { container } = render(
      <InputWrapper {...defaultProps} variant="filled" />
    );
    expect(
      container.querySelector(".raikou-InputWrapper-root")
    ...
    expect(inputWrapperQueries.getLabel(container)).toHaveAttribute(
      "data-variant",
      "filled"
    );
  });
