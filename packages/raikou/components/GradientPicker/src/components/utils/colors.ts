...
import { colorNameKeys } from "./color-names";
import { regRgb, regRgba, regHsl, regHsla } from "./regexp";
 ...
 * @since 1.0.0
 */
const sanitizeAlpha = (alpha: string | number) => {
  return parseFloat(
    Math.max(
      Math.min(
        typeof alpha === "string" ? parseFloat(alpha as string) : alpha,
        1,
      ),
      0,
    )
...
const rgbHslString = (
  ...
  const r = Math.round(parseInt(rr));
  const g = Math.round(parseInt(gg));
  const b = Math.round(parseInt(bb));
 ...
 * @since 1.0.0
 */
const rgbToHex = (r: string, g: string, b: string, reduce = true) => {
 ...
 * @since 1.0.0
 */
const isValidColor = (color: string, varyingLength: boolean) => {
  return (
    isValidHex(color, varyingLength) ||
 ...
 * @since 1.0.0
 */
const reduceHex = (clr: string, fromOutput = 3) => {
 ...
 * @since 1.0.0
 */
const hexToRGB = (color: string, opacity: number = 100) => {
  let hex = color.replace("#", "");
  if (hex.length === 3) {
    hex = fullHex(hex);
  }
 ...
 * @since 1.0.0
 */
const getRgbHslValues = (color: string, hsl: boolean) => {
  ...
  const values = clr
    .substring(clr.indexOf("(") + 1, clr.lastIndexOf(")"))
    .split(",");
  ...
  for (let i = 0; i < 4; i++) {
    if (i < 3) {
      ...
      values[i] = Math.min(max, Math.max(0, parseInt(values[i], 10)));
    } else {
      values[i] = Math.min(1, Math.max(0, parseFloat(values[i])));
    }
  }
 ...
 * @since 1.0.0
 */
const getValuesFromHex = (color: string) => {
  let hex;
 ...
 * @since 1.0.0
 */
const isValidHex = (color: string, varyingNumbers: boolean) => {
  if (!varyingNumbers) {
 ...
 * @since 1.0.0
 */
const cssColor = (color: string | string[]) => {
  if (typeof color === "string") {
    ...
    return rgbHslString(color[0], color[1], color[2]);
  }
 ...
 * @since 1.0.0
 */
const getRawHex = (value: string | string[]) => {
 ...
 * @since 1.0.0
 */
const verifyColorBySettings = (
  clr: string,
  mode: "single" | "gradients",
  regGradient: RegExp,
  allowConic: boolean,
) => {
  if (mode !== "single") {
