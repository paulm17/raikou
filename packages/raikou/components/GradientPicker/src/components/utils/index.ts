...
import { defaultGradient, maxPositionPixels } from "./defaults";
...
import { calcGradPosition } from "./common";
 ...
 * @since 1.0.0
 */
const getGradientObject = (color: string) => {
 ...
 * @since 1.0.0
 */
const getGradients = (grad: string) => {
  ...
  return list.map((gradient) => gradientObject(gradient))[0] as GradientObj;
};
 ...
 * @since 1.0.0
 */
const gradientObject = (gradient: string) => {
  ...
  let newGrad: any = grad.join(",").replace(/\)$/, "");
  newGrad = createGradientColorsArray(newGrad);
  newGrad = filterEmpties(newGrad);

  const { colors, hints } = getGradientColors(newGrad, type);
 ...
 * @since 1.0.0
 */
const getGradientData = (type: GradientType, prefix: string) => {
 ...
 * @since 1.0.0
 */
const createGradientColorsArray = (gradient: string) => {
  const colors = [];
 ...
 * @since 1.0.0
 */
const filterEmpties = (colors: string[]) => {
 ...
 * @since 1.0.0
 */
const getGradientColors = (clrs: any, type: GradientType) => {
  const conic = type === "conic";
  let colors = clrs.map((color: any) => verifyGradColor(color, conic));
 ...
 * @since 1.0.0
 */
const calculateHints = (colors: any[], hints: any) => {
 ...
 * @since 1.0.0
 */
const formatGradientPositions = (clr: any, type: GradientType) => {
  if (!isNaN(clr)) {
    return [clr];
  }
 ...
 * @since 1.0.0
 */
const removeConsecutiveHints = (colors: any[]) => {
 ...
 * @since 1.0.0
 */
const verifyGradColor = (clr: string, conic: boolean) => {
...
const writeGradientColor = (
  ...
  return {
    ...
    position,
    opacity: colorArray[3] ? colorArray[3] : 1,
    hex: rgbToHex(colorArray[0], colorArray[1], colorArray[2]),
    rgb: [
      parseInt(colorArray[0]),
      parseInt(colorArray[1]),
      parseInt(colorArray[2]),
