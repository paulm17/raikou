 ...
 * @since 1.0.0
 */
const radialGradientData = (prefix: string) => {
 ...
 * @since 1.0.0
 */
const getRadialShape = (shape: string) => {
  if (shape.search(/ellipse|circle/) === -1) {
    let prefix = shape.split("at");
    if (prefix.length > 1) {
      const nprefix = prefix[0].trim();
      if (!nprefix || nprefix.search(/side|corner/) !== -1) {
        return "ellipse";
      }
      return "size";
    }
 ...
 * @since 1.0.0
 */
const getRadialExtent = (grad: string) => {
 ...
 * @since 1.0.0
 */
const getRadialSizes = (prefix: string, shape: string) => {
  if (shape !== "size") {
    return {
      x: { value: 75, unit: "%" },
      y: { value: 75, unit: "%" },
...
const getRadialPosition = (angle: string) => {
