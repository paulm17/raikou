/* eslint-disable */
import { cssColor, reduceHex } from "./colors";
import { toFixed, trimComma } from "./utilities";
 ...
 * @since 1.0.0
 */
const allColorsEqual = (gradients: any[]) => {
  return !gradients.filter((gradient) => {
    const { colors } = gradient;
    const { color: firstColor } = colors[0];
    return !colors.every((obj: any) => obj.color === firstColor);
  }).length;
};
...
const gradientColor = (gradColor, hint, conic) => {
  const { unit, opacity, position, color: colorValue } = gradColor;
...
const radialGradient = (grad) => {
  ...
  if (!defaultX || !defaultY) {
    ...
    if (defaultY && posXUnit === "%") {
      ...
      gradient += `at ${toFixed(posXValue)}${posXUnit} ${toFixed(
        posYValue,
      )}${posYUnit}, `;
    }
  } else {
