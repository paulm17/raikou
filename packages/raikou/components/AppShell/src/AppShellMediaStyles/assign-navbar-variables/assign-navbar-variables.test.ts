import { DEFAULT_THEME, px, rem } from "@raikou/core";
...
describe("@raikou/core/AppShell/assign-navbar-variables", () => {
  describe("assigns correct base styles", () => {
    ...
    it("no options", () => {
      expect(
        getTestObject({ width: 100, breakpoint: "sm" }).baseStyles
      ).toStrictEqual({
    ...
    it("base width", () => {
      expect(
        getTestObject({ width: { base: 100 }, breakpoint: "sm" }).baseStyles
      ).toStrictEqual({
        "--app-shell-navbar-width": rem(100),
        "--app-shell-navbar-offset": rem(100),
      });
  ...
  describe("assigns correct max media styles", () => {
    it("sets correct breakpoint variables", () => {
      expect(
        getTestObject({ width: 100, breakpoint: "sm" }).maxMediaStyles
      ).toStrictEqual({
        sm: {
    ...
    it("sets correct variables when navbar is collapsed on mobile", () => {
      expect(
        ...
        [(px(DEFAULT_THEME.breakpoints.sm) as number) - 0.1]: {
          ...
          "--app-shell-navbar-transform-rtl":
