...
describe("@raikou/core/AppShell/assign-header-variables", () => {
  describe("assigns correct base styles", () => {
    ...
    it("responsive height", () => {
      expect(
        getTestObject({ height: { base: 100, sm: 200 } }).baseStyles
      ).toStrictEqual({
        "--app-shell-header-height": rem(100),
        "--app-shell-header-offset": rem(100),
      });
    ...
    it("offset: false", () => {
      expect(
        getTestObject({ height: 100, offset: false }).baseStyles
      ).toStrictEqual({
        "--app-shell-header-height": rem(100),
      });
    });
    ...
    it("collapsed: true", () => {
      expect(
        getTestObject({ height: 100, collapsed: true }).baseStyles
      ).toStrictEqual({
        "--app-shell-header-height": rem(100),
    ...
    it("offset: false, collapsed: true", () => {
      expect(
        getTestObject({ height: 100, collapsed: true, offset: false })
          .baseStyles
      ).toStrictEqual({
        "--app-shell-header-height": rem(100),
        "--app-shell-header-offset": "0px !important",
  ...
  describe("assigns correct min media styles", () => {
    it("assigns correct height", () => {
      expect(
        getTestObject({ height: { base: 100, sm: 200, lg: 300 } })
          .minMediaStyles
      ).toStrictEqual({
        sm: {
