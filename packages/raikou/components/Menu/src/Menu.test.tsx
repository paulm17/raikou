...
import { tests, render, screen, userEvent } from "@raikou/tests";
...
describe("@raikou/core/Menu", () => {
  ...
  it("correctly handles withArrow={true}", () => {
    ...
    expect(document.querySelectorAll(".raikou-Menu-arrow")).toHaveLength(1);
  });

  it("correctly handles withArrow={false}", () => {
    render(<TestContainer defaultOpened withArrow={false} />);
    expect(document.querySelectorAll(".raikou-Menu-arrow")).toHaveLength(0);
  });
  ...
  it("correctly calls callbacks when opening and closing the uncontrolled menu via target click", async () => {
    ...
    render(
      <TestContainer onChange={onChange} onOpen={onOpen} onClose={onClose} />,
    );
    expectClosed();
