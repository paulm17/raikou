...
var varsResolver6 = createVarsResolver6(
  (_, { size }) => ({
    chevron: {
      "--combobox-chevron-size": getSize2(size, "combobox-chevron-size")
    }
  })
);
...
function NativeSelectOption({ data }) {
  if (isGroup(data)) {
    const items = data.items.map((item) => /* @__PURE__ */ React12.createElement(NativeSelectOption, { key: item.value, data: item }));
    return /* @__PURE__ */ React12.createElement("optgroup", { label: data.group }, items);
  }
  const _a = data, { value, label } = _a, others = __objRest(_a, ["value", "label"]);
  return /* @__PURE__ */ React12.createElement("option", __spreadValues({ key: data.value, value: data.value }, others), data.label);
}
...
var NativeSelect = factory7((props, ref) => {
  const _a = useProps9(
    "NativeSelect",
    defaultProps9,
    props
  ), { data, children, size, error, rightSection } = _a, others = __objRest(_a, ["data", "children", "size", "error", "rightSection"]);
  const options = getParsedComboboxData(data).map((item, index) => /* @__PURE__ */ React13.createElement(NativeSelectOption, { key: index, data: item }));
  return /* @__PURE__ */ React13.createElement(
    ...
    __spreadProps(__spreadValues({
      ...
      pointer: true,
      error,
      rightSection: rightSection || /* @__PURE__ */ React13.createElement(ComboboxChevron, { size, error })
    }),
    children || options
  );
});
