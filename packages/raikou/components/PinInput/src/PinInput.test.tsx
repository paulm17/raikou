...
describe("@raikou/core/PinInput", () => {
  ...
  it("renders correct number of inputs based on length prop", () => {
    ...
    expect(container.querySelectorAll(".raikou-PinInput-input")).toHaveLength(
      5,
    );
  });
  ...
  it("onComplete is called on last input", () => {
    ...
    const { container } = render(
      <PinInput {...defaultProps} onComplete={spy} />,
    );
  ...
  it("stay focused on last element on initial backspace press", async () => {
    ...
    container
      .querySelectorAll(".raikou-PinInput-input")
      .forEach((element, key) => {
    ...
    expect(
      container.querySelectorAll(".raikou-PinInput-input")[4],
    ).toHaveFocus();
  });
  ...
  it("inputs except the last element should not need two backspace press to shift focus to prev element", async () => {
    ...
    await userEvent.keyboard("{Backspace}");
    expect(
      container.querySelectorAll(".raikou-PinInput-input")[3],
    ).toHaveFocus();

    await userEvent.keyboard("{Backspace}");
    expect(
      container.querySelectorAll(".raikou-PinInput-input")[2],
    ).toHaveFocus();

    await userEvent.keyboard("{Backspace}");
    expect(
      container.querySelectorAll(".raikou-PinInput-input")[1],
    ).toHaveFocus();

    await userEvent.keyboard("{Backspace}");
    expect(
      container.querySelectorAll(".raikou-PinInput-input")[0],
    ).toHaveFocus();
  });
  ...
  it("inputs will be filled when the value is bigger than 2 chars(ex: Gboard paste action from keypad)", async () => {
    const spy = jest.fn();
    const { container } = render(
      <PinInput {...defaultProps} onComplete={spy} length={6} />,
    );
