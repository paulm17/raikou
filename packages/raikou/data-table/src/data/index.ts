...
export const departments: Department[] = departmentData.map(
  ({ companyId, ...rest }) => ({
    ...rest,
...
export const employees: Employee[] = employeeData.map(
  ({ departmentId, ...rest }) => ({
    ...rest,
...
export async function countCompanyDepartmentsAsync({
  ...
  await delay(delayOptions);
  return departments.filter((department) => department.company.id === companyId)
    .length;
}
...
export async function countCompanyEmployeesAsync({
  ...
  return employees.filter(
    (employee) => employee.department.company.id === companyId,
  ).length;
}
