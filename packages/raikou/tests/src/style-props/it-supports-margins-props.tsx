...
export function itSupportsMarginsProps<Props>(
  ...
  it(name, () => {
    const { container: m } = render(
      <options.component {...options.props} m={45} />
    );
    const { container: theme } = render(
      <options.component {...options.props} m="xl" />
    );
    const { container: mx } = render(
      <options.component {...options.props} mx={34} />
    );
    const { container: my } = render(
      <options.component {...options.props} my={22} />
    );
    const { container: mt } = render(
      <options.component {...options.props} mt={13} />
    );
    const { container: mb } = render(
      <options.component {...options.props} mb={43} />
    );
    const { container: mr } = render(
      <options.component {...options.props} mr={98} />
    );
    const { container: ml } = render(
      <options.component {...options.props} ml={11} />
    );

    expect(m.querySelector(selector)).toHaveStyle({ margin: rem(45) });
    expect(theme.querySelector(selector)).toHaveStyle({
      margin: "var(--raikou-spacing-xl)",
    });
    expect(mx.querySelector(selector)).toHaveStyle({
      marginLeft: rem(34),
      marginRight: rem(34),
    });
    expect(my.querySelector(selector)).toHaveStyle({
      marginTop: rem(22),
      marginBottom: rem(22),
    });
