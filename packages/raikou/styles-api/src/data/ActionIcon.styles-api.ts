import type { ActionIconFactory, ActionIconGroupFactory } from "@raikou/server";
...
export const ActionIconStylesApi: StylesApiData<ActionIconFactory> = {
  selectors: {
    root: "Root element",
    loader:
      "`Loader` component, rendered inside root element when `loading` prop is set",
  },
  ...
  vars: {
    root: {
      ...
      "--ai-size":
        "Controls `width`, `height`, `min-width` and `min-height` styles",
    },
  },
