import type { ColorSwatchFactory } from "@raikou/server";
...
export const ColorSwatchStylesApi: StylesApiData<ColorSwatchFactory> = {
  ...
  vars: {
    root: {
      "--cs-radius":
        "Controls `border-radius` of all overlays and `root` element",
      "--cs-size":
        "Controls `width`, `height`, `min-width` and `min-height` of the `root` element",
    },
  },
};
