...
import type { FactoryPayload, TransformVars } from "@raikou/core";
...
export interface Selectors<Factory extends FactoryPayload> {
  selectors: Factory["stylesNames"] extends string
    ? Record<Factory["stylesNames"], string>
    : never;
}
...
export interface Modifiers<Factory extends FactoryPayload> {
  modifiers?: Factory["stylesNames"] extends string
    ? Modifier<Factory["stylesNames"]>[]
    : never;
}

export type StylesApiData<Factory extends FactoryPayload> =
  (Factory["stylesNames"] extends string ? Selectors<Factory> : EmptyObject) &
    Vars<Factory["vars"]> &
