...
import { getRaikouPackagesList } from '../packages/get-packages-list';
import { publishPackage } from '../publish/publish-package';
import { getPath } from '../utils/get-path';
import { createLogger } from '../utils/signale';
import { openGithubRelease } from './open-github-release';
import { setRaikouPackagesVersion } from './set-raikou-packages-version';
...
async function release() {
  ...
  await setRaikouPackagesVersion(incrementedVersion);
  ...
  const raikouPackages = await getRaikouPackagesList();

  await Promise.all(
    raikouPackages.map((p) =>
